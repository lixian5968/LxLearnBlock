...
public class MultiImageSelector {
    ...
    public void start(Activity activity, int requestCode,boolean cameraNoCallBack){
        final Context context = activity;
        if(hasPermission(context)) {
            activity.startActivityForResult(createIntent(context,cameraNoCallBack), requestCode);
        }else{
    ...
    public void start(Activity activity, int requestCode){
        final Context context = activity;
        if(hasPermission(context)) {
            activity.startActivityForResult(createIntent(context,false), requestCode);
        }else{
            Toast.makeText(context, R.string.mis_error_no_permission, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void start(Fragment fragment, int requestCode){
        ...
        if(hasPermission(context)) {
            fragment.startActivityForResult(createIntent(context,false), requestCode);
        }else{
            Toast.makeText(context, R.string.mis_error_no_permission, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private Intent createIntent(Context context,boolean cameraNoCallBack){
        ...
        intent.putExtra("cameraNoCallBack", cameraNoCallBack);
        if(mOriginData != null){
